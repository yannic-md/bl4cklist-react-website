 ...
 */
export default function MemberList({members, position = 'right', planetVariant = 1}: MemberListProps): JSX.Element {
    ...
    const getPlanetConfigs = (): Array<{ src: string; className: string; alt: string }> => {
        const baseConfigs = {
            ...
            2: [
                {
                    src: '/images/bg/moon.svg',
                    className: `absolute ${isLeftPosition ? 'left-0 bottom-40' : 'right-0 bottom-40'} z-[3] pointer-events-none opacity-20`,
                    alt: 'Moon Decoration Primary'
                },
                {
                    src: '/images/bg/moon.svg',
                    className: `absolute ${isLeftPosition ? 'right-5 top-72' : 'left-5 top-72'} z-[3] pointer-events-none opacity-25`,
                    alt: 'Moon Decoration Secondary'
                }
            ],
            3: [
    ...
    return (
        <section className="relative py-16 md:py-24 lg:py-36 bg-slate-900/30" id="birthdays">
                    ...
                    <div className={`flex justify-start items-start flex-col gap-3 sm:gap-4 lg:gap-5 xl:sticky xl:top-36 
                        ...
                        <div className="font-bold tracking-wider w-full">
                            <AnimateOnView animation={`animate__fadeIn${isLeftPosition ? 'Right' : 'Left'} animate__slower`}>
                                <AnimatedTextReveal text={tMemberListSection('infoTagBirthday')}
                        ...
                        <AnimateOnView animation={`animate__fadeIn${isLeftPosition ? 'Right' : 'Left'} animate__slower 
                            ...
                            <h2 className={`${index.head_border} bg-clip-text text-transparent mb-2
                                ...
                                <span className="inline-block align-middle leading-none -mx-[3px] sm:-mx-[5px]
                                                 -mt-[3px] sm:-mt-[5px] text-white text-2xl sm:text-3xl">🍰</span> - {tMemberListSection('titleBirthday')}
                            </h2>
                        </AnimateOnView>
                        ...
                        <AnimateOnView animation={`animate__fadeIn${isLeftPosition ? 'Right' : 'Left'} animate__slower`}>
                            <p className={`text-[#969cb1] pt-2 sm:pt-3 break-words max-w-full xl:max-w-lg text-xs 
                                           sm:text-sm text-center lg:text-start xl:text-${isLeftPosition ? 'end' : 'start'} 
                                           md:text-base mx-auto xl:mx-0 `}>
                                {tMemberListSection('description1Birthday')}
                                <br /><br />
                                {tMemberListSection.rich('description2Birthday', {
                                    code: (chunks: ReactNode): JSX.Element => (
                                        <code className="font-mono bg-slate-800/70 px-1 sm:px-1.5 py-0.5 rounded text-[0.85em] sm:text-[0.9em]">
                                            {chunks}
                                        </code>
                                    )
                                })}
